...
int main(int argc, char *argv[])
{
    ...
    if (result != 0)
    {
        close(opts.in_sock_fd);
        close(opts.out_sock_fd);
        fatal_message(__FILE__, __func__, __LINE__, "[FAIL] initiate server's sockaddr_in", EXIT_FAILURE);
    }
    ...
    if (result == THREAD_FAIL)
    {
        close(opts.in_sock_fd);
        close(opts.out_sock_fd);
        fatal_message(__FILE__, __func__, __LINE__, "[FAIL] create thread", EXIT_FAILURE);
    }
    else if (result != 0)
    {
        close(opts.in_sock_fd);
        close(opts.out_sock_fd);
        fatal_message(__FILE__, __func__, __LINE__, "[FAIL] do_proxy", EXIT_FAILURE);
    }
